...
int rtapi_app_main(void) 
{
    ...
    if ((res = hal_pin_float_new("align-gantry-kins.x-cent", HAL_IN, &(align_pins->x_cent), comp_id)) < 0) goto error;
    X_CENT = 0;
    if ((res = hal_pin_float_new("align-gantry-kins.y-cent", HAL_IN, &(align_pins->y_cent), comp_id)) < 0) goto error;
    Y_CENT = 0;
    if ((res = hal_pin_float_new("align-gantry-kins.x-offset", HAL_IN, &(align_pins->x_offset), comp_id)) < 0) goto error;
    // g5x offset
    X_OFFSET = 0;
    if ((res = hal_pin_float_new("align-gantry-kins.y-offset", HAL_IN, &(align_pins->y_offset), comp_id)) < 0) goto error;
    Y_OFFSET = 0;
    // align_pins->theta = 0;
