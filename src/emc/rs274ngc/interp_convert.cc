...
int Interp::comp_get_current(setup_pointer settings, double *x, double *y,
			     double *z)
{
...
int Interp::comp_set_current(setup_pointer settings, double x, double y,
			     double z)
{
    switch (settings->plane) {
    case CANON_PLANE_XY:
...
int Interp::comp_get_programmed(setup_pointer settings, double *x,
				double *y, double *z)
{
    switch (settings->plane) {
    case CANON_PLANE_XY:
...
int Interp::comp_set_programmed(setup_pointer settings, double x, double y,
				double z)
{
    switch (settings->plane) {
    case CANON_PLANE_XY:
			  ...
			  setup_pointer settings)	//!< pointer to machine settings
{
	...
	if ((block->x_flag) && (block->y_flag)) {
	    CHP(find_ends
		(block, settings, &CP.X, &CP.Y, &end_z, &AA_end, &BB_end,
		 &CC_end, &u_end, &v_end, &w_end));
	    CP.W = block->p_number;
	    nurbs_control_points.push_back(CP);
	}
			   ...
			   setup_pointer settings)	//!< pointer to machine settings
{
	...
	CHP(find_ends
	    (block, settings, &x3, &y3, &end_z, &AA_end, &BB_end, &CC_end,
	     &u_end, &v_end, &w_end));
			...
			setup_pointer settings)	//!< pointer to machine settings             
{
	...
	if ((settings->cutter_comp_side == OFF) ||
		...
		convert_arc_comp1(move, block, settings, end_x, end_y,
				  end_z, block->i_number, block->j_number,
				  AA_end, BB_end, CC_end, u_end, v_end,
				  w_end);
	    CHP(status);
	} else {
	    status =
	...
	if ((settings->cutter_comp_side == OFF) ||
	    ...
	    status =
		convert_arc_comp1(move, block, settings, end_z, end_x,
				  end_y, block->k_number, block->i_number,
				  AA_end, BB_end, CC_end, u_end, v_end,
				  w_end);
	    CHP(status);
	} else {
	    status =
		convert_arc_comp2(move, block, settings, end_z, end_x,
				  end_y, block->k_number, block->i_number,
				  AA_end, BB_end, CC_end, u_end, v_end,
				  w_end);
			 ...
			 double offset2)
{
    ...
    inverse_time_rate_arc(*current1, *current2, *current3, center1,
			  center2, turn, end1, end2, end3, block,
			  settings);
...
int Interp::convert_arc_comp1(int move,	//!< either G_2 (cw arc) or G_3 (ccw arc)            
			      ...
			      double offset_x, double offset_y, double AA_end,	//!< a-value at end of arc                     
			      double BB_end,	//!< b-value at end of arc                     
			      double CC_end,	//!< c-value at end of arc                     
			      double u_end, double v_end, double w_end)	//!< uvw at end of arc
{
    ...
    tolerance =
	(settings->length_units ==
	 CANON_UNITS_INCHES) ? TOLERANCE_INCH : TOLERANCE_MM;
    ...
    if (block->r_flag) {
	CHP(arc_data_comp_r
	    (move, plane, side, tool_radius, cx, cy, end_x, end_y,
	     block->r_number, &center_x, &center_y, &turn, tolerance));
    } else {
	CHP(arc_data_comp_ijk
	    (move, plane, side, tool_radius, cx, cy, end_x, end_y,
	     (settings->ijk_distance_mode == MODE_ABSOLUTE), offset_x,
	     offset_y, &center_x, &center_y, &turn, tolerance));
    }
	...
	enqueue_STRAIGHT_FEED(settings, block->line_number, 0, 0, 0, cx,
			      cy, cz, AA_end, BB_end, CC_end, u_end, v_end,
			      w_end);
	set_endpoint(cx, cy);
    }

    enqueue_ARC_FEED(settings, block->line_number,
		     find_turn(cx, cy, center_x, center_y, turn, end_x,
			       end_y), end_x, end_y, center_x, center_y,
		     turn, end_z, AA_end, BB_end, CC_end, u_end, v_end,
		     w_end);
			      ...
			      double u, double v, double w)	//!< uvw at end of arc
{
	...
	CHP(arc_data_r(move, plane, opx, opy, end_x, end_y,
		       block->r_number, &centerx, &centery, &turn,
		       tolerance));
    } else {
    ...
    beta =
	(beta > (1.5 * M_PIl)) ? (beta - (2 * M_PIl)) : (beta <
							 -M_PI_2l) ? (beta
								      +
								      (2 *
								       M_PIl))
	...
	if (qc().front().type != QARC_FEED) {
	    ...
	    double oldrad =
		hypot(prev.center2 - prev.end2, prev.center1 - prev.end1);
	    double newrad;
	    if TOOL_INSIDE_ARC
		(side, turn) {
	    ...
	    CHKS((oldrad == 0
		  || arc_cc == 0),
		 _
		 ("Arc to arc motion is invalid because the arcs have the same center"));
	    a = (SQ(oldrad) + SQ(arc_cc) -
		 SQ(newrad)) / (2 * oldrad * arc_cc);

	    CHKS((a > 1.0
		  || a < -1.0),
		 (_
		  ("Arc to arc motion makes a corner the compensated tool can't fit in without gouging")));
	    pullback = acos(a);
	...
	enqueue_ARC_FEED(settings, block->line_number,
			 find_turn(opx, opy, centerx, centery, turn, end_x,
				   end_y), new_end_x, new_end_y, centerx,
			 centery, turn, end_z, AA_end, BB_end, CC_end, u,
			 v, w);
	...
	enqueue_ARC_FEED(settings, block->line_number, 0.0,	// doesn't matter since we won't move this arc's endpoint
			 midx, midy, opx, opy, ((side == LEFT) ? -1 : 1),
			 cz, AA_end, BB_end, CC_end, u, v, w);
	dequeue_canons(settings);
				 ...
				 setup_pointer settings)	//!< pointer to machine settings                   
{
	...
	settings->u_current =
	    (settings->u_current + settings->u_axis_offset);
	settings->v_current =
	    (settings->v_current + settings->v_axis_offset);
	settings->w_current =
	    (settings->w_current + settings->w_axis_offset);
	SET_ORIGIN_OFFSETS(settings->origin_offset_x,
			   settings->origin_offset_y,
			   settings->origin_offset_z,
			   settings->AA_origin_offset,
	...
	settings->current_x =
	    settings->current_x + settings->axis_offset_x -
	    USER_TO_PROGRAM_LEN(pars[5211]);
	settings->current_y =
	    settings->current_y + settings->axis_offset_y -
	    USER_TO_PROGRAM_LEN(pars[5212]);
	settings->current_z =
	    settings->current_z + settings->axis_offset_z -
	    USER_TO_PROGRAM_LEN(pars[5213]);
	settings->AA_current =
	    settings->AA_current + settings->AA_axis_offset -
	    USER_TO_PROGRAM_ANG(pars[5214]);
	settings->BB_current =
	    settings->BB_current + settings->BB_axis_offset -
	    USER_TO_PROGRAM_ANG(pars[5215]);
	settings->CC_current =
	    settings->CC_current + settings->CC_axis_offset -
	    USER_TO_PROGRAM_ANG(pars[5216]);
	settings->u_current =
	    settings->u_current + settings->u_axis_offset -
	    USER_TO_PROGRAM_LEN(pars[5217]);
	settings->v_current =
	    settings->v_current + settings->v_axis_offset -
	    USER_TO_PROGRAM_LEN(pars[5218]);
	settings->w_current =
	    settings->w_current + settings->w_axis_offset -
	    USER_TO_PROGRAM_LEN(pars[5219]);
...
{
    enum { LC_SIZE = 256, EX_SIZE = 2 * LC_SIZE };	// 256 from comment[256] in rs274ngc.hh
    char lc[LC_SIZE + 1];
	...
	convert_param_comment(comment + start + strlen(DEBUG_STR),
			      expanded, EX_SIZE);
	MESSAGE(expanded);
	return INTERP_OK;
    } else if (startswith(lc, PRINT_STR)) {
	convert_param_comment(comment + start + strlen(PRINT_STR),
			      expanded, EX_SIZE);
	fprintf(stdout, "%s\n", expanded);
	fflush(stdout);
	return INTERP_OK;
				 ...
				 setup_pointer settings)	//!< pointer to machine settings                 
{
    CHKS((settings->cutter_comp_side != OFF),
	 (_
	  ("Cannot change control mode with cutter radius compensation on")));
    if (g_code == G_61) {
				      ...
				      setup_pointer settings)	//!< pointer to machine settings                  
{
    ...
    CHKS((settings->cutter_comp_side != OFF),
	 (_
	  ("Cannot change coordinate systems with cutter radius compensation on")));
...
#ifdef DEBUG_EMC
	enqueue_COMMENT
	    ("interpreter: continuing to use same coordinate system");
#endif
	return INTERP_OK;
    }
    ...
    find_current_in_system(settings, origin,
			   ...
			   &settings->current_z, &settings->AA_current,
			   &settings->BB_current, &settings->CC_current,
			   &settings->u_current, &settings->v_current,
			   &settings->w_current);
	...
	USER_TO_PROGRAM_LEN(parameters[5201 + (origin * 20)]);
    settings->origin_offset_y =
	USER_TO_PROGRAM_LEN(parameters[5202 + (origin * 20)]);
    settings->origin_offset_z =
	USER_TO_PROGRAM_LEN(parameters[5203 + (origin * 20)]);
    settings->AA_origin_offset =
	USER_TO_PROGRAM_ANG(parameters[5204 + (origin * 20)]);
    settings->BB_origin_offset =
	USER_TO_PROGRAM_ANG(parameters[5205 + (origin * 20)]);
    settings->CC_origin_offset =
	USER_TO_PROGRAM_ANG(parameters[5206 + (origin * 20)]);
    settings->u_origin_offset =
	USER_TO_PROGRAM_LEN(parameters[5207 + (origin * 20)]);
    settings->v_origin_offset =
	USER_TO_PROGRAM_LEN(parameters[5208 + (origin * 20)]);
    settings->w_origin_offset =
	USER_TO_PROGRAM_LEN(parameters[5209 + (origin * 20)]);
					   ...
					   setup_pointer settings)	//!< pointer to machine settings              
{
	...
	if (block->l_number != -1) {
	    CHKS((settings->plane != CANON_PLANE_XZ),
		 _("G%d.1 with L word, but plane is not G18"),
		 block->g_modes[7] / 10);
	...
	radius =
	    USER_TO_PROGRAM_LEN(settings->tool_table[index].diameter) /
	    2.0;
	orientation = settings->tool_table[index].orientation;
	CHKS((settings->plane != CANON_PLANE_XZ && orientation != 0
	      && orientation != 9),
	     _("G%d with lathe tool, but plane is not G18"),
	     block->g_modes[7] / 10);
    }
    ...
    if (side == RIGHT)
	enqueue_COMMENT
	    ("interpreter: cutter radius compensation on right");
    else
...
#ifdef DEBUG_EMC
	    enqueue_COMMENT
		("interpreter: distance mode changed to absolute");
#endif
...
#ifdef DEBUG_EMC
	    enqueue_COMMENT
		("interpreter: distance mode changed to incremental");
#endif
...
#ifdef DEBUG_EMC
	    enqueue_COMMENT
		("interpreter: IJK distance mode changed to absolute");
#endif
	    settings->ijk_distance_mode = MODE_ABSOLUTE;
	}
...
#ifdef DEBUG_EMC
	    enqueue_COMMENT
		("interpreter: IJK distance mode changed to incremental");
#endif
...
#ifdef DEBUG_EMC
	    COMMENT
		("interpreter: Lathe diameter mode changed to diameter");
#endif
...
#ifdef DEBUG_EMC
	enqueue_COMMENT
	    ("interpreter: feed mode set to units per revolution");
#endif
		      ...
		      setup_pointer settings)	//!< pointer to machine settings                 
{
	...
	CHP(convert_lathe_diameter_mode
	    (block->g_modes[15], block, settings));
    }
    if (block->g_modes[7] != -1) {
	CHP(convert_cutter_compensation
	    (block->g_modes[7], block, settings));
    }
    if (block->g_modes[8] != -1) {
	CHP(convert_tool_length_offset
	    (block->g_modes[8], block, settings));
    }
    if (block->g_modes[12] != -1) {
	CHP(convert_coordinate_system(block->g_modes[12], settings));
    }
    if (block->g_modes[13] != -1) {
	CHP(convert_control_mode
	    (block->g_modes[13], block->p_number, block->q_number,
	     settings));
    }
...
int Interp::convert_savehome(int code, block_pointer block,
			     setup_pointer s)
{
    ...
    double x =
	PROGRAM_TO_USER_LEN(s->current_x + s->tool_xoffset +
			    s->origin_offset_x + s->axis_offset_x);
    double y =
	PROGRAM_TO_USER_LEN(s->current_y + s->origin_offset_y +
			    s->axis_offset_y);
    double z =
	PROGRAM_TO_USER_LEN(s->current_z + s->tool_zoffset +
			    s->origin_offset_z + s->axis_offset_z);
    double a =
	PROGRAM_TO_USER_ANG(s->AA_current + s->AA_origin_offset +
			    s->AA_axis_offset);
    double b =
	PROGRAM_TO_USER_ANG(s->BB_current + s->BB_origin_offset +
			    s->BB_axis_offset);
    double c =
	PROGRAM_TO_USER_ANG(s->CC_current + s->CC_origin_offset +
			    s->CC_axis_offset);
    double u =
	PROGRAM_TO_USER_LEN(s->u_current + s->u_origin_offset +
			    s->u_axis_offset);
    double v =
	PROGRAM_TO_USER_LEN(s->v_current + s->v_origin_offset +
			    s->v_axis_offset);
    double w =
	PROGRAM_TO_USER_LEN(s->w_current + s->w_origin_offset +
			    s->w_axis_offset);
				 ...
				 setup_pointer settings)	//!< pointer to machine settings                 
{
	...
	if (settings->length_units != CANON_UNITS_INCHES) {
	    ...
	    settings->u_axis_offset =
		(settings->u_axis_offset * INCH_PER_MM);
	    settings->v_axis_offset =
		(settings->v_axis_offset * INCH_PER_MM);
	    settings->w_axis_offset =
		(settings->w_axis_offset * INCH_PER_MM);
	    settings->u_origin_offset =
		(settings->u_origin_offset * INCH_PER_MM);
	    settings->v_origin_offset =
		(settings->v_origin_offset * INCH_PER_MM);
	    settings->w_origin_offset =
		(settings->w_origin_offset * INCH_PER_MM);
	...
	if (settings->length_units != CANON_UNITS_MM) {
	    ...
	    settings->u_axis_offset =
		(settings->u_axis_offset * MM_PER_INCH);
	    settings->v_axis_offset =
		(settings->v_axis_offset * MM_PER_INCH);
	    settings->w_axis_offset =
		(settings->w_axis_offset * MM_PER_INCH);
	    settings->u_origin_offset =
		(settings->u_origin_offset * MM_PER_INCH);
	    settings->v_origin_offset =
		(settings->v_origin_offset * MM_PER_INCH);
	    settings->w_origin_offset =
		(settings->w_origin_offset * MM_PER_INCH);
		      ...
		      setup_pointer settings)	//!< pointer to machine settings                 
{
	...
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot set motion output with cutter radius compensation on")));	// XXX
	...
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot set motion digital output with cutter radius compensation on")));	// XXX
	...
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot set auxiliary digital output with cutter radius compensation on")));	// XXX
	...
	CHKS(((block->q_number <= 0) && (block->l_flag == ON)
	      && (round_to_int(block->l_number) > 0)),
	     NCE_ZERO_TIMEOUT_WITH_WAIT_NOT_IMMEDIATE);

	// E-word specified (analog input) and wait type not immediate
	CHKS(((block->e_flag == ON) && (block->l_flag == ON)
	      && (round_to_int(block->l_number) != 0)),
	...
	if (block->p_flag == ON) {	// got a digital input
	    ...
	    CHKS((settings->cutter_comp_side != OFF),
		 (_
		  ("Cannot wait for digital input with cutter radius compensation on")));

	    int ret =
		WAIT(round_to_int(block->p_number), DIGITAL_INPUT, type,
		     timeout);
	    ...
	    CHKS((settings->cutter_comp_side != OFF),
		 (_
		  ("Cannot wait for analog input with cutter radius compensation on")));
	...
	//E-word = analog channel
	//Q-word = analog value
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot set motion analog output with cutter radius compensation on")));	// XXX
	CHKS((block->e_flag == OFF)
	     || (round_to_int(block->e_number) < 0),
	     (_("Invalid analog index with M67")));
	SET_MOTION_OUTPUT_VALUE(round_to_int(block->e_number),
				block->q_number);
    } else if (block->m_modes[5] == 68) {
	//E-word = analog channel
	//Q-word = analog value
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot set auxiliary analog output with cutter radius compensation on")));	// XXX
	CHKS((block->e_flag == OFF)
	     || (round_to_int(block->e_number) < 0),
	     (_("Invalid analog index with M68")));
	SET_AUX_OUTPUT_VALUE(round_to_int(block->e_number),
			     block->q_number);
    }
	...
	if (block->m_modes[6] == 61) {
	    CHKS((round_to_int(block->q_number) < 0),
		 (_
		  ("Need positive Q-word to specify tool number with M61")));
...
*/
	...
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot enable overrides with cutter radius compensation on")));	// XXX
	ENABLE_FEED_OVERRIDE();
	ENABLE_SPEED_OVERRIDE();
	settings->feed_override = ON;
	settings->speed_override = ON;
    } else if (block->m_modes[9] == 49) {
	CHKS((settings->cutter_comp_side != OFF), (_("Cannot disable overrides with cutter radius compensation on")));	// XXX
...
int Interp::convert_probe(block_pointer block,	//!< pointer to a block of RS274 instructions
			  int g_code, setup_pointer settings)	//!< pointer to machine settings             
{
    double end_x;
    double end_y;
    double end_z;
    ...
    CHKS((settings->feed_rate == 0.0),
	 NCE_CANNOT_PROBE_WITH_ZERO_FEED_RATE);
    CHP(find_ends
	(block, settings, &end_x, &end_y, &end_z, &AA_end, &BB_end,
	 &CC_end, &u_end, &v_end, &w_end));
    CHKS(((!(probe_type & 1)) && settings->current_x == end_x
	  && settings->current_y == end_y && settings->current_z == end_z
	  && settings->AA_current == AA_end
	  && settings->BB_current == BB_end
	  && settings->CC_current == CC_end && settings->u_current == u_end
	  && settings->v_current == v_end
	  && settings->w_current == w_end),
	 NCE_START_POINT_TOO_CLOSE_TO_PROBE_POINT);
...
int Interp::convert_setup_tool(block_pointer block, setup_pointer settings)
{
    ...
    CHKS((block->y_flag || block->a_flag || block->b_flag || block->c_flag
	  || block->u_flag
	  || block->v_flag), "Invalid axis specified for G10 L1");

    CHKS((GET_EXTERNAL_TLO_IS_ALONG_W()
	  && block->z_flag), "Found Z but expected W in G10 L1");
    CHKS((!GET_EXTERNAL_TLO_IS_ALONG_W()
	  && block->w_flag), "Found W but expected Z in G10 L1");

    if (block->q_number != -1.0) {
	CHKS((!is_near_int(&q, block->q_number)),
	     "Q number in G10 is not an integer");
	CHKS((block->x_flag
	      && q == 0),
	     "Cannot have an X tool offset with orientation 0");
	...
	if (block->l_number == 10)
	    z = settings->current_z + settings->tool_zoffset - z;
	settings->tool_table[pocket].zoffset = PROGRAM_TO_USER_LEN(z);
    } else if (block->w_flag) {
	double w = block->w_number;
	if (block->l_number == 10)
	    w = settings->w_current + settings->tool_woffset - w;
	...
	if (block->l_number == 10)
	    x = settings->current_x + settings->tool_xoffset - x;
    ...
    if (!_setup.random_toolchanger && toolno == settings->current_pocket) {
	if (settings->tool_table[pocket].orientation) {
			  ...
			  setup_pointer settings)	//!< pointer to machine settings                 
{
    ...
    CHKS((block->l_number == 20 && block->a_flag
	  && settings->a_axis_wrapped && (block->a_number <= -360.0
					  || block->a_number >= 360.0)),
	 (_("Invalid absolute position %5.2f for wrapped rotary axis %c")),
	 block->a_number, 'A');
    CHKS((block->l_number == 20 && block->b_flag
	  && settings->b_axis_wrapped && (block->b_number <= -360.0
					  || block->b_number >= 360.0)),
	 (_("Invalid absolute position %5.2f for wrapped rotary axis %c")),
	 block->b_number, 'B');
    CHKS((block->l_number == 20 && block->c_flag
	  && settings->c_axis_wrapped && (block->c_number <= -360.0
					  || block->c_number >= 360.0)),
	 (_("Invalid absolute position %5.2f for wrapped rotary axis %c")),
	 block->c_number, 'C');

    find_current_in_system(settings, p_int,
			   &cx, &cy, &cz, &ca, &cb, &cc, &cu, &cv, &cw);
	...
	// find new offset
	x = oldx + USER_TO_PROGRAM_LEN(parameters[5201 + (p_int * 20)]) -
	    x;
	y = oldy + USER_TO_PROGRAM_LEN(parameters[5202 + (p_int * 20)]) -
	    y;
...
int Interp::convert_set_plane(int g_code,	//!< must be G_17, G_18, or G_19 
			      setup_pointer settings)	//!< pointer to machine settings 
{
    CHKS((settings->cutter_comp_side != OFF && g_code == G_17
	  && settings->plane != CANON_PLANE_XY),
	 NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
    CHKS((settings->cutter_comp_side != OFF && g_code == G_18
	  && settings->plane != CANON_PLANE_XZ),
	 NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
...
int Interp::convert_spindle_mode(block_pointer block,
...
/*1*/
	...
	settings->origin_offset_x =
	    USER_TO_PROGRAM_LEN(settings->parameters[5221]);
	settings->origin_offset_y =
	    USER_TO_PROGRAM_LEN(settings->parameters[5222]);
	settings->origin_offset_z =
	    USER_TO_PROGRAM_LEN(settings->parameters[5223]);
	settings->AA_origin_offset =
	    USER_TO_PROGRAM_ANG(settings->parameters[5224]);
	settings->BB_origin_offset =
	    USER_TO_PROGRAM_ANG(settings->parameters[5225]);
	settings->CC_origin_offset =
	    USER_TO_PROGRAM_ANG(settings->parameters[5226]);
	settings->u_origin_offset =
	    USER_TO_PROGRAM_LEN(settings->parameters[5227]);
	settings->v_origin_offset =
	    USER_TO_PROGRAM_LEN(settings->parameters[5228]);
	settings->w_origin_offset =
	    USER_TO_PROGRAM_LEN(settings->parameters[5229]);
			     ...
			     setup_pointer settings)	//!< pointer to machine settings             
{
	...
	inverse_time_rate_straight(end_x, end_y, end_z,
				   AA_end, BB_end, CC_end,
				   u_end, v_end, w_end, block, settings);
    }
	...
	if (settings->plane == CANON_PLANE_XZ) {
		...
		status =
		    convert_straight_comp1(move, block, settings, end_z,
					   end_x, end_y, AA_end, BB_end,
					   CC_end, u_end, v_end, w_end);
	    else
		status =
		    convert_straight_comp2(move, block, settings, end_z,
					   end_x, end_y, AA_end, BB_end,
					   CC_end, u_end, v_end, w_end);
	} else if (settings->plane == CANON_PLANE_XY) {
	    if (settings->cutter_comp_firstmove == ON)
		status =
		    convert_straight_comp1(move, block, settings, end_x,
					   end_y, end_z, AA_end, BB_end,
					   CC_end, u_end, v_end, w_end);
	    else
		status =
		    convert_straight_comp2(move, block, settings, end_x,
					   end_y, end_z, AA_end, BB_end,
					   CC_end, u_end, v_end, w_end);
	} else
	    ERS("BUG: Invalid plane for cutter compensation");
	CHP(status);
    } else if (move == G_0) {
	STRAIGHT_TRAVERSE(block->line_number, end_x, end_y, end_z,
			  AA_end, BB_end, CC_end, u_end, v_end, w_end);
	...
	STRAIGHT_FEED(block->line_number, end_x, end_y, end_z, AA_end,
	...
	CHKS((settings->AA_current != AA_end ||
	      ...
	      settings->v_current != v_end ||
	      settings->w_current != w_end),
	     NCE_CANNOT_MOVE_ROTARY_AXES_WITH_G76);
	int result =
...
threading_pass(setup_pointer settings, block_pointer block,
	       ...
	       double start_y, double start_z, double zoff,
	       double taper_dist, int entry_taper, int exit_taper,
	       double taper_pitch, double pitch, double full_threadheight,
	       double target_z)
{
    STRAIGHT_TRAVERSE(block->line_number, boring ? safe_x + depth - end_depth : safe_x - depth + end_depth, start_y, start_z - zoff, AABBCC);	//back
    if (taper_dist && entry_taper) {
	DISABLE_FEED_OVERRIDE();
	START_SPEED_FEED_SYNCH(taper_pitch, 0);
	STRAIGHT_FEED(block->line_number, boring ? safe_x + depth - full_threadheight : safe_x - depth + full_threadheight, start_y, start_z - zoff, AABBCC);	//in
	...
	STRAIGHT_TRAVERSE(block->line_number, boring ? safe_x + depth : safe_x - depth, start_y, start_z - zoff, AABBCC);	//in
	DISABLE_FEED_OVERRIDE();
	START_SPEED_FEED_SYNCH(pitch, 0);
    }
	...
	START_SPEED_FEED_SYNCH(taper_pitch, 0);
	STRAIGHT_FEED(block->line_number, boring ? safe_x + depth - full_threadheight : safe_x - depth + full_threadheight, start_y, target_z - zoff, AABBCC);	//angled out
    } else {
	STRAIGHT_FEED(block->line_number, boring ? safe_x + depth : safe_x - depth, start_y, target_z - zoff, AABBCC);	//over
    }
    STOP_SPEED_FEED_SYNCH();
    STRAIGHT_TRAVERSE(block->line_number, boring ? safe_x + depth - end_depth : safe_x - depth + end_depth, start_y, target_z - zoff, AABBCC);	//out
    ENABLE_FEED_OVERRIDE();
}
				    ...
				    double end_z)
{
    ...
    CHKS((settings->cutter_comp_side != OFF),
	 (_
	  ("Cannot use G76 threading cycle with cutter radius compensation on")));

    CHKS((block->i_number == 0), (_("In G76, I must not be 0")));
    CHKS((block->j_number <= 0), (_("In G76, J must be greater than 0")));
    ...
    if (degression < 1.0 || !block->r_flag)
	degression = 1.0;
	...
	threading_pass(settings, block, boring, safe_x, depth, end_depth,
		       ...
		       target_z);
	depth =
	    full_dia_depth + cut_increment * pow(++pass, 1.0 / degression);
	zoff = (depth - full_dia_depth) * tan(compound_angle);
    }
				   ...
				   double w_end)
{
    ...
    CHKS(((side != LEFT)
	  && (side != RIGHT)), NCE_BUG_SIDE_NOT_RIGHT_OR_LEFT);
    CHKS((distance <= radius),
	 "Length of cutter compensation entry move is not greater than the tool radius");

    alpha =
	atan2(py - cy,
	      px - cx) + (side == LEFT ? M_PIl / 2. : -M_PIl / 2.);
				   ...
				   double w_end)
{
	...
	if (!concave && (beta > small)) {	/* ARC NEEDED */
	    CHP(move_endpoint_and_flush(settings, cx, cy));
	    if (move == G_1) {
		enqueue_ARC_FEED(settings, block->line_number, 0.0,	// doesn't matter, since we will not move this arc's endpoint
				 mid_x, mid_y, opx, opy,
				 ((side == LEFT) ? -1 : 1), cz,
				 AA_end, BB_end, CC_end, u_end, v_end,
				 w_end);
		...
		CHKS((halfcorner == 0.0),
		     (_
		      ("Zero degree inside corner is invalid for cutter compensation")));
		...
		double oldrad =
		    hypot(prev.center2 - prev.end2,
			  prev.center1 - prev.end1);
		...
		theta =
		    (prev.turn ==
		     1) ? base_dir + M_PI_2l : base_dir - M_PI_2l;
		...
		if TOOL_INSIDE_ARC
		    ...
		    CHKS((l > 1.0
			  || l < -1.0),
			 _
			 ("Arc to straight motion makes a corner the compensated tool can't fit in without gouging"));
		    if (prev.turn == 1)
...
int Interp::convert_tool_change(setup_pointer settings)	//!< pointer to machine settings
{
	...
	find_relative(0., 0., 0., 0., 0., 0., 0., 0., 0.,
		      ...
		      &discard, &discard, &discard, settings);
	COMMENT("AXIS,hide");
	STRAIGHT_TRAVERSE(-1, settings->current_x, settings->current_y,
			  up_z, settings->AA_current, settings->BB_current,
				       ...
				       setup_pointer settings)	//!< pointer to machine settings                 
{
    ...
    CHKS((settings->cutter_comp_side != OFF),
	 (_
	  ("Cannot change tool offset with cutter radius compensation on")));
    if (g_code == G_49) {
	xoffset = 0.;
	zoffset = 0.;
	woffset = 0.;
	...
	if (GET_EXTERNAL_TLO_IS_ALONG_W()) {
	    woffset =
		USER_TO_PROGRAM_LEN(settings->tool_table[index].zoffset);
	    zoffset = 0.;
	} else {
	    zoffset =
		USER_TO_PROGRAM_LEN(settings->tool_table[index].zoffset);
